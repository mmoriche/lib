...
elseif strcmp(wildpath,'**')
  ...
  D = misc.rdir([prepath postpath(2:end)]);
  ...
  c_D = arrayfun(@(x) misc.rdir([prepath x.name filesep wildpath postpath]),...
  ...
  if ~isdir(prepath) || ( numel(D_sd)==1 && strcmp(D_sd.name, prepath))
    % Fix case like misc.rdir('path*\...') where prepath is not a full directoty
  ...
  c_D = arrayfun(@(x) misc.rdir([prepath x.name postpath]),...
    D_sd, 'UniformOutput', false);
